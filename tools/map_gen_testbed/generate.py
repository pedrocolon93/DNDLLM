...
# ------------------------------------------------------------------ big-slice
def big_slice_prompt(theme: str, size: int, bible: str, layout_ascii: str) -> str:
...
LAYOUT ({size}x{size} grid, y=0 is top; symbols: .=Floor, #=Wall, +=Door, B=Building):
{layout_ascii}

CRITICAL RULES:
- Render the ENTIRE {size}x{size} map as ONE continuous picture with NO internal frames, NO grid lines, NO tile borders.
- Orthographic top-down view. No perspective, no vignette, no title card, no caption.
...
- Buildings (B) are standalone structures on the floor, drawn to fit inside their cell.
